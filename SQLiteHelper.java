...
public class SQLiteHelper {
    ...
    public static void insertInfo(String AccountName, String UserName, String Password, String Notes) {
        ...
        try (Connection conn = DriverManager.getConnection(SQLiteContract.URL);
            ...
            pstmt.setInt(5, PassGuardLoginController.getCurrentUser()); //this gets the primary key of the current user
            pstmt.executeUpdate();
        } catch (SQLException e) {
            System.out.println(e.getMessage());
        }
    ...
    public static ArrayList<String> getAllAccounts() {
        ...
        try (Connection conn = DriverManager.getConnection(SQLiteContract.URL);
            ...
            pstmt.setInt(1, PassGuardLoginController.getCurrentUser());//this restricts the current user to only accessing their data
     ...
     *
     * @param AccountName - name of the account that corresponds to the notes being changed
     * @param newNotes - new notes for the specified account
     *
     */
    public static void updateNotes(String AccountName, String newNotes) {
        
        String SQL_UPDATE_NOTES =
        "UPDATE " + SQLiteContract.UserInfo.TABLE_NAME +
        " SET " + SQLiteContract.UserInfo.COLUMN_NOTES +" = ? WHERE "
        + SQLiteContract.UserInfo.COLUMN_ACCOUNT + " = ?";
        
        try (Connection conn = DriverManager.getConnection(SQLiteContract.URL);
             PreparedStatement pstmt = conn.prepareStatement(SQL_UPDATE_NOTES)) {
    ...
    /**
     * Changes the stored account name for a specified account in the SQLite Database
     *
     * @param AccountName - name of the account that corresponds to the account being changed
     * @param newName - new account name for a specified account
     *
     */
    public static void updateAccount(String AccountName, String newAccount) {
        
        String SQL_UPDATE_ACCOUNT =
        "UPDATE " + SQLiteContract.UserInfo.TABLE_NAME +
        " SET " + SQLiteContract.UserInfo.COLUMN_ACCOUNT +" = ? WHERE "
        + SQLiteContract.UserInfo.COLUMN_ACCOUNT + " = ?";
        
        try (Connection conn = DriverManager.getConnection(SQLiteContract.URL);
             PreparedStatement pstmt = conn.prepareStatement(SQL_UPDATE_ACCOUNT)) {
            ...
            LOGGER.log(Level.SEVERE, "Account name was not updated in the UserInfo Table");
            LOGGER.log(Level.SEVERE, e.getMessage());
            System.out.println(e.getMessage());
        }
    }
     ...
     *
     * @param Username - Username of the corresponding account being deleted
     * @param Password - Password of the corresponding account being deleted
     * @param AccountName - Account name that is being deleted
     *
     */
    public static void deleteAccount(String Username, String Password, String AccountName) {
        
       'DELETE FROM' + SQLiteContract.UserInfo.TABLE_NAME + ' = ? WHERE '
        + SQLiteContract.UserInfo.COLUMN_ACCOUNT + ' = ?'
     ...
     *
     * @param Username - Username of the profile being deleted
     * @param Password - Password of the profile being deleted
     *
     */
    public static void deleteProfile(String Username, String Password) {
        
        'DELETE FROM ' + SQLiteContract.UserInfo.TABLE_NAME + ' = ? WHERE '
        + SQLiteContract.UserInfo.COLUMN_ACCOUNT + " = ?" + SQLiteContract.UserInfo.COLUMN_USERNAME + " = ?"+ SQLiteContract.UserInfo.COLUMN_PASSWORD + " = ?"+ SQLiteContract.UserInfo.COLUMN_NOTES + " = ?"+ SQLiteContract.UserInfo.COLUMN_USERID + " = ?";
    }
}
